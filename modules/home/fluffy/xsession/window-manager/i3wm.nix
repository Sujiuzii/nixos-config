...
let
  polybar-config = pkgs.fluffy.polybar-config;
  cfg = config.fluffy.home.xsession.window-manager.i3;
in with lib; {
  imports = [
    ./app-menu
    ./bar
  ...
  options = {
    fluffy.home.xsession.window-manager.i3 = {
  ...
  config = {
    xsession.windowManager.i3 = mkIf (pkgs.stdenv.isLinux && cfg.enable) {
      enable = true;
      config = {
        bars = [];
        gaps.inner = 9;
